...
3. You will use this application for educational or private purposes only`,
  ...
  static async hasUserConsented(): Promise<boolean> {
    try {
      ...
      const consentData = JSON.parse(await fs.promises.readFile(this.consentFilePath, 'utf-8'));
  ...
  static async showConsentDialog(parentWindow?: BrowserWindow): Promise<boolean> {
    ...
    const options = {
      ...
      detail: consentText,
      buttons: [isJapanese ? '同意しない' : 'Decline', isJapanese ? '同意する' : 'Accept'],
      defaultId: 0,
      cancelId: 0,
      noLink: true,
    };
  ...
  private static async saveConsent(): Promise<void> {
    ...
    try {
      await fs.promises.writeFile(this.consentFilePath, JSON.stringify(consentData, null, 2));
    } catch (error) {
  ...
  static async getConsentInfo(): Promise<any | null> {
    try {
      ...
      return JSON.parse(await fs.promises.readFile(this.consentFilePath, 'utf-8'));
    } catch (error) {
