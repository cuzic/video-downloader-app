...
export async function initDatabase(): Promise<void> {
  try {
    ...
    if (deletedBackups > 0) {
      await auditLogRepo.info('database', 'backup_cleanup', `Deleted ${deletedBackups} old backups`);
    }
...
function setupPeriodicMaintenance(): void {
  ...
  const dailyBackup = setInterval(async () => {
    ...
    if (hour === 3) {
      try {
        ...
        await auditLogRepo.info('database', 'scheduled_backup', `Scheduled backup created: ${backupPath}`);
  ...
  const weeklyCleanup = setInterval(async () => {
    ...
    if (day === 0 && hour === 4) { // Sunday at 4 AM
      try {
        const { taskRepo, historyRepo, detectionRepo, statisticsRepo, auditLogRepo: auditRepo } = 
