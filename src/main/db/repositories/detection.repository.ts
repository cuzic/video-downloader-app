...
export class DetectionRepository {
  async create(detection: NewDetection): Promise<string> {
    await db.insert(detections).values(detection)
      .onConflictDoUpdate({
  ...
  async listByPageUrl(pageUrl: string): Promise<Detection[]> {
    return db.select()
      .from(detections)
      .where(and(
        eq(detections.pageUrl, pageUrl),
        eq(detections.autoDelete, 0)
      ))
      .orderBy(desc(detections.detectedAt));
  }
  ...
  async cleanupOld(daysOld = 7): Promise<number> {
    ...
    const result = await db.delete(detections)
      .where(and(
        ...
        lte(detections.lastSeenAt, cutoff)
      ));
