...
export class AuditLogRepository {
  ...
  async getInRange(startTime: number, endTime: number): Promise<AuditLog[]> {
    return db
      .select()
      .from(auditLogs)
      .where(and(gte(auditLogs.timestamp, startTime), lte(auditLogs.timestamp, endTime)))
      .orderBy(auditLogs.timestamp);
  }
  ...
  async logDownloadEvent(taskId: string, event: string, details?: any): Promise<void> {
    await this.log('info', 'download', event, undefined, details, taskId);
  }

  // Helper to log detection events
  async logDetectionEvent(event: string, url: string, details?: any): Promise<void> {
    await this.info('detection', event, url, details);
  }

  // Helper to log security events
  async logSecurityEvent(event: string, message: string, details?: any): Promise<void> {
