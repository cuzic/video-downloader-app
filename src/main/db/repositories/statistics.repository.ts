...
export class StatisticsRepository {
  ...
  async recordDownload(
    ...
    const canceledCount = isCanceled ? 1 : 0;
    ...
    await db
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          completedCount:
            completedCount > 0
              ? (sql`${statistics.completedCount} + 1` as any)
              : (statistics.completedCount as any),
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          errorCount:
            errorCount > 0
              ? (sql`${statistics.errorCount} + 1` as any)
              : (statistics.errorCount as any),
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          canceledCount:
            canceledCount > 0
              ? (sql`${statistics.canceledCount} + 1` as any)
              : (statistics.canceledCount as any),
  ...
  async getRange(startDate: string, endDate: string): Promise<Statistics[]> {
    return db
      .select()
      .from(statistics)
      .where(and(gte(statistics.date, startDate), lte(statistics.date, endDate)))
  ...
  async getMediaTypeStats(
    days = 30
  ): Promise<Array<{ mediaType: string; count: number; bytes: number }>> {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - days);
    const cutoff = this.formatDate(cutoffDate);
  ...
  async cleanup(daysOld = 365): Promise<number> {
    ...
    const result = await db.delete(statistics).where(lte(statistics.date, cutoff));
